...
export function createTypegooseProviders(
  ...
  const createDiscriminatorFactoryFrom =
    (parentFactory: ModelFactory) =>
  ...
  return models.reduce(
    (
      providers,
      { typegooseClass, schemaOptions = {}, discriminators = [] },
    ) => {
      ...
      const discriminatorProviders = discriminators.map(
        createDiscriminatorFactoryFrom(modelFactory),
      );
...
/**
 * Sanitizes the input to a common object form used for creating providers.
 * @param item varied general input to convert
 * @returns a sanitized generic form
 * @internal
 */
export function convertToTypegooseClassWithOptions(
  item: TypegooseInput,
): TypegooseClassWithOptions {
...
/**
 * Returns whether something is a class.
 * @param item the value to check whether it is a class
 * @internal
 */
const isTypegooseClass = (item): item is TypegooseClass => isClass(item);

/**
 * Returns whether a value is a typegoose class with options.
 * @param item the value to check whether it is a typegoose class with options
 * @internal
 */
...
function convertToOptions(
  item: TypegooseInput,
): ClassOrDiscriminator | undefined {
